...
if [ -f "comparativa-backend/.env" ]; then
    ...
    sed -i "s|FRONTEND_URL=.*|FRONTEND_URL=https://$DDNS_DOMAIN|g" comparativa-backend/.env
...
REACT_APP_API_URL=https://$DDNS_DOMAIN/api
...
if [ -f "comparativa-backend/server.js" ]; then
    ...
    sed -i "s|'http://TU_DDNS_AQUI:3000'|'https://$DDNS_DOMAIN'|g" comparativa-backend/server.js
    sed -i "s|\"http://192.168.1.82:4000\"|\"https://$DDNS_DOMAIN\"|g" comparativa-backend/server.js
...
echo "✅ Backend: https://$DDNS_DOMAIN"
echo ""
echo "📋 Próximos pasos:"
